...
- Ansible and OpenStack SDK, e.g. `pip install ansible openstacksdk`
...
- SSH keypair, path customizable with Ansible variables `ssh_path` and `user_pri_key`
...
2. Clone this role `tripleo-ci-reproducer` to [`ANSIBLE_ROLES_PATH`](
   ...
   If you want to clone this repository to a different directory, add a symbolic link in `ANSIBLE_ROLES_PATH` to this
   repository, for example:

   ```sh
   mkdir -p ~/.ansible/roles
   git clone https://github.com/rdo-infra/ansible-role-tripleo-ci-reproducer.git
   ln -s ansible-role-tripleo-ci-reproducer ~/.ansible/roles/tripleo-ci-reproducer
   ```
...
clouds:
  rdo-cloud:
    ...
    auth:
      auth_url: http://my.cloud.authurl.org/identity
...
- name: Set up reproducer
  ...
  vars:
    ...
    upstream_gerrit_user: "<your_upstream_gerrit_user>"
    rdo_gerrit_user: "<your_rdo_gerrit_user>"
  tasks:
    - name: Set up reproducer
      include_role:
