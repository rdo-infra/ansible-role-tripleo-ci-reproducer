...
Gerrit. Follow these 3 steps to install:
  ...
  3. Run [start.yaml](#start-playbook) playbook to set up reproducer tenant

```bash
ansible-playbook -vv start.yaml -e "rdo_gerrit_username=<your_upstream_gerrit_user> rdo_gerrit_username=<your_rdo_gerrit_user>"
```
...
```bash
ansible-playbook -vv start.yaml -e "upstream_gerrit_username=<your_upstream_gerrit_user> rdo_gerrit_username=<your_rdo_gerrit_user>"
```
